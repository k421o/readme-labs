...
## README review capability release candidate gates
...
These gates protect the versioned `readme-review` finding and no-finding job.
They do not yet define a promotion contract for `readme-generate`. Generation
promotion requires its own versioned scenario matrix and independent semantic
evidence that generated bytes pass the complete review workflow, stay inside
the declared write boundary, and preserve residual limits without inventing
facts.

Pruning promotion requires its own writable scenario matrix. It must exercise
directed removals, review-evidenced removal and no-op outcomes, restoration of
agent-selected over-deletion, whole-file refusal, exact diff scoring, and
residual-limit reporting without exposing expected results to the capability.
The current contract, adapter, and deterministic tests establish addressable
behavior but not pruning efficacy. The `0.3.0-dev.2` adapter remains unreleased
until both generation and pruning have their own promotion contracts and
evidence.
...
- Each generated skill tree is byte-identical to its pinned canonical source.
